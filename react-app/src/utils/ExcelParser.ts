...
import type { ProjectData, ProjectStatus, TaskData, BurnupRow } from '../types';
import { addDays, isAfter, startOfToday, subDays, parse } from 'date-fns';
...
const normalizeStatus = (status: string, completionPercent: number = 0, isLate: boolean = false): ProjectStatus => {
    ...
    return 'Não Iniciado'; // Default fallback
};
        ...
        reader.onload = (e) => {
            try {
                ...
                workbook.SheetNames.forEach((sheetName) => {
                    ...
                    // Check for Burnup Sheet (by name OR content)
                    let isBurnup = sheetName.toLowerCase().includes('controle de projetos') || sheetName.toLowerCase().includes('burnup');

                    // If name doesn't match, check content for specific headers
                    if (!isBurnup) {
                        const grid = XLSX.utils.sheet_to_json<any[]>(sheet, { header: 1, defval: '' });
                        if (grid && grid.length > 0) {
                            // Check first 10 rows for "Plano Base" and "Real"
                            for (let r = 0; r < Math.min(grid.length, 10); r++) {
                                const rowStr = (grid[r] || []).join(' ').toUpperCase();
                                if (rowStr.includes('PLANO BASE') && (rowStr.includes('REAL') || rowStr.includes('ATUAL'))) {
                                    isBurnup = true;
                                    break;
                                }
                            }
                    ...
                    if (isBurnup) {
                        const burnupData = parseBurnupSheet(sheet);
                        if (burnupData && burnupData.length > 0) {
                            // Create a dummy project to hold this data, or attach to a specific project if we knew which one.
                            // For now, we'll create a "Master Project" that holds this portfolio view.
                            projects.push({
                                id: 'master-burnup-' + Date.now(),
                                name: 'Consolidado (Burnup)',
                                status: 'Em Andamento',
                                saving: 0,
                                tasks: [],
                                stageDurations: {},
                                startDate: burnupData[0].date,
                                baselineDate: burnupData[burnupData.length - 1].date,
                                actualDate: null,
...
const parseProjectSheet = (sheet: XLSX.WorkSheet, sheetName: string, fileName: string): ProjectData | null => {
    ...
    for (let r = 0; r < Math.min(grid.length, 15); r++) {
        ...
        for (let c = 0; c < row.length; c++) {
            ...
            // Technology (Tecnologia, Tech, Stack)
            if (!technology) {
                if (lowerVal.includes('tecnologia')) technology = extractHeaderValue(row, c, 'tecnologia');
                else if (lowerVal.includes('tech')) technology = extractHeaderValue(row, c, 'tech');
                else if (lowerVal.includes('stack')) technology = extractHeaderValue(row, c, 'stack');
            }
        }
    }
...
const parseBurnupSheet = (sheet: XLSX.WorkSheet): BurnupRow[] | null => {
    const grid = XLSX.utils.sheet_to_json<any[]>(sheet, { header: 1, defval: '' });
    ...
    for (let r = 0; r < 10; r++) {
        const rowStr = (grid[r] || []).join(' ').toUpperCase();
        if (rowStr.includes('MÊS') || rowStr.includes('MES') || rowStr.includes('PLANO BASE')) {
            headerRowIndex = r;
            break;
        }
    }
    ...
    const headerRow = grid[headerRowIndex].map(v => String(v).toUpperCase().trim());
    const colIndexDate = headerRow.findIndex(h => h.includes('MÊS') || h.includes('MES'));
    const colIndexBaseline = headerRow.findIndex(h => h.includes('PLANO BASE'));
    const colIndexRealized = headerRow.findIndex(h => h.includes('REAL') || h.includes('ATUAL') && !h.includes('PLANO'));
    const colIndexForecast = headerRow.findIndex(h => h.includes('PREVISÃO') || h.includes('FORECAST') || h.includes('PLANO ATUAL'));

    if (colIndexDate === -1 || colIndexBaseline === -1) return null;

    const burnupData: BurnupRow[] = [];

    for (let r = headerRowIndex + 1; r < grid.length; r++) {
        const row = grid[r];
        if (!row) continue;

        const dateVal = row[colIndexDate];
        let date: Date | null = null;

        if (typeof dateVal === 'number') {
            date = parseExcelDate(dateVal);
        } else if (typeof dateVal === 'string') {
            // Try parsing "jan-26" format
            try {
                // Check if it matches MMM-yy
                if (dateVal.match(/^[a-z]{3}-\d{2}$/i)) {
                    date = parse(dateVal, 'MMM-yy', new Date(), { locale: ptBR });
                } else {
                    date = parseExcelDate(dateVal);
                }
            } catch (e) {
                console.warn("Date parse error", dateVal);
            }
        }

        if (!date || isNaN(date.getTime())) continue;

        const baseline = typeof row[colIndexBaseline] === 'number' ? row[colIndexBaseline] : parseFloat(row[colIndexBaseline]) || 0;

        let realized: number | null = null;
        if (colIndexRealized > -1) {
            const val = row[colIndexRealized];
            if (typeof val === 'number') realized = val;
            else if (val && typeof val === 'string' && val.trim() !== '') realized = parseFloat(val);
        }

        let forecast: number | null = null;
        if (colIndexForecast > -1) {
            const val = row[colIndexForecast];
            if (typeof val === 'number') forecast = val;
            else if (val && typeof val === 'string' && val.trim() !== '') forecast = parseFloat(val);
        }
    ...
    return burnupData.sort((a, b) => a.date.getTime() - b.date.getTime());
};

// Wrapper class for compatibility
export class ExcelParser {
    static async parseFile(file: File): Promise<ProjectData[]> {
